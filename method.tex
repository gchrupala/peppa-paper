...
and fit a linear model with a number of predictors which may
correspond to control variables as well as hypothesized relevant factors.
...
the regression targets. The predictors are: whether the two word
tokens correspond to the same word type, whether they are uttered by
the same speaker, whether they occur in the same episode,
phoneme-level normalized edit distance between the tokens, difference
in duration between the two tokens, and the cosine similarity between
the word-type-embeddings corresponding to the two word-tokens
(specifically GloVe word-vectors). For this analysis we exclude word
pairs where either word is out of vocabulary for the GloVe dataset. In
order to obtain model embeddings for single-word utterances, we use
forced-alignment of audio with corresponding subtitles, discarding
tokens where alignment fails. For the dialog condition the number of
pairwise scores for the remaining tokens is 453,628; for the narration
condition it is 2,7250,653.
...
pretrained model is available via Pytorch.\footnote{See \url{https://pytorch.org/vision/0.8/models.html\#resnet-3d}.}  The
