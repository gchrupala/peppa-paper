...
at chance \citep[see also][]{nikolaus-fourtassi-2021-evaluating}.\todo{GC: I
  find the notation here a bit confusing, especially the overloaded
  one-letter names. Maybe simplify. MN: I tried my best, do you think it's more 
  comprehensive now?}
...
an attention mechanism with dimensionwise \todo{MN: I got a bit stuck here 
because I was unsure what the dimensions for "dimensionwise weights" are. It's 
the output dimensions of wav2vec, that are not directly interpretable, right? 
Maybe we should mention this difference with respect to Merkx et al. 2019? And 
maybe we can mention the dimensionality (28?)} weights 
\citep{Merkx2019}:
\begin{equation}
  \begin{aligned}
